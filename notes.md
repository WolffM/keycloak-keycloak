...
2. Inspect `common/src/main/java/org/keycloak/common/Profile.java` and locate the `DYNAMIC_SCOPES` feature entry in the `Profile.Feature` enum.
3. Inspect `js/apps/admin-ui/src/context/server-info/__tests__/mock.json` to verify how the feature is classified in mocked server info.
